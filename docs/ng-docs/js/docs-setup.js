NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$interval _attacheventlisteners _createmetaobj _drawplayerdiv _emitstatechange _formatplayerstatechangeevent _gethtml5videoobject _initplayerdiv _onstatechange _player abstract abstracts api appropriate array asset attached attaching availble base boolean bound buffered buffering called class conforms copy copying corresponds create creating current currently data determine determines divid effects element embed embeds emit emitterid ended event events evetn file files format freeze freezemetaprops freezes function functionality getbufferedpercent getcurrenttime getinstance getplayerstate handle handler helper html html5 implements individual input instance interface interger invoke itt itt-service-html5playermanager-page itt-service-page ittutils listeners m3u8 main mainplayer mapped media mediasrcarr message meta metaobj method methods mp4 mute note numerical object objects onbuffering oncanplay onended onpause onplaying onwaiting optional pause paused percent pid pipe play playback playbackrate playbackservice played player playermanager playermanagers playerstate playerstatechange playerstates playing prior proper properties props provided quality query rate ready reduces relevant remove reports representing responds return returns seedplayermanager seek select send service set setspeed setting setvolume side side-effect slow source sources speed src statechangecallback stream string strings tag tags time timelinesvc togglemute toggles type ultimately unfreeze unfreezemetaprops unique urls video videoelement videoelm videoobj videos void volume webm wiring wraps youtube yt"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "$location _createinstance _emitstatechange _formatplayerstatechange _getpidfrominstance _getplayerdiv _players _reset _tick _trycommand api app array asset buffered buffering callback called cb change changing clear clock command completed create cued current currently data debugging desired destroy destroyinstance detecting determine determines div divid dom download element embed embedded embeds emiiterid emitted empty ended entire error errors errorsvc event events finished fire fired function getcurrenttime getmetaobj getplayerdiv getter getvideoloadedfraction handler handling html https iframe iframes input instance instances int interaction interface invoke itt ittvideo js level main mainplayer manually map mediasrcarr meta metadata metaobj method mute note number numerical object objects onerror onplayerqualitychange onplayerstatechange onqualitychange onready onreadycb onstatechange operation optional optionally param parameter pause paused percent pick pid play playback playbackrate player playerid playerstate playerstatechange playerstates playervar playing prior properties props quality qualitychangecb range rate ready register registerstatechangelistener representation representing requested reset resetplayermanager responsible restart resume resumes retrieve return returns running safari seconds seedplayermanager seek seeking seekto server service set setplaybackquality setspeed setter setvolume slow source specifies speed statechangecb stops stream string svc target time timelinesvc togglemute toggles type unique unregister unregisterstatechangelistener unstarted urls val video videoid videos void volume working youtube youtubeplayerapi youtubeurlservice yt ytinstance"
    },
    {
      "section": "api",
