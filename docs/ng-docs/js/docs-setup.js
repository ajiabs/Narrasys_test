NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$destory $interval _emitstatechange _getplayermanagerfrommediasrc _id _mainplayerid _onplayerready _playerinterfaces _players _setpid _statechangecb accept aka api app array asset blank bufferedpercent buffering called case cb change command commands consumes create createinstance creating data db derive destroyed determines directive duration embed embeds emit emitted emitterid emitting entry episode event events example exported exports fired freezemetaprops function getcurrenttime getmetaprop getplayerdiv getplayerstate gettimelinestate handle handles helper initialization input instance interface interfaces interval invoke invokes itt ittvideo left level listener listeners loops main mainplayer mainplayerid mainvideo map masterasset media mediasrcarr meta method methods object onready optional param parameter parsedmediasrcarr passed pause pauseotherplayers percent pid play playback playbackrate playbackservice player playerid playermanager playermanagers playerstates playerstates_word playing playvideo prior prop properties property queries query rate ready record registered registerstatechangelistener relevant remove removes responded resume return returns saved seedplayer seek service set setmetaprop setspeed settimelinestate setup setvolume side-effects speed startat startattime statechangeevent statechangelistener statechanges statement store stores string switch syncs time timeline timelinestate timelinesvc timlinesvc togglemute track type ui unfreezemetaprops unregisterstatechangelistener upstream urls val video videos void volume wires youtube"
    },
    {
      "section": "api",
