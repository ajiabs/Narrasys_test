...
function VideoCtrlBeta($q, $scope, $timeout, playbackService, timelineSvc, appState, playbackChannel, PLAYERSTATES, STATECHANGE) {
	...
	function initPlayer() {
		...
		playbackService.setPlayer($scope.video, _id, true);
		timelineSvc.registerVideo($scope);
	}

	function play() {
		return playbackService.play(_id);
	}

	function pause() {
		return playbackService.pause(_id);
	}

	function seek() {
		playbackService.seek(_id, appState.time);
	}
...
function VideoController($q, $scope, $rootScope, $timeout, $interval, $window, $document, appState, timelineSvc, analyticsSvc, youTubePlayerManager, html5PlayerManager) {
	...
	function playHtml5() {
		return html5PlayerManager.play($scope.videoNode.id);
	}
