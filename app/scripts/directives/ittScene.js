		...
		return {
			...
			link: function (scope, element, attrs) {
				...
				var twiddleScene = function () {
					// console.log("twiddleScene called for ",scope.scene);
					$timeout(function () { // wait for any DOM updates first
						// Adjust layout: only trigger on active scenes or on first explore-mode scene (HACK)
				...
				var sceneAutoScroll = function () {
					// Don't scroll in explore mode; ittItem will handle that.
					if (scope.scene.templateUrl.indexOf("scene-explore") === -1 && scope.scene.isActive) {
				...
				if ($rootScope.isFramed && ($rootScope.isIPad || $rootScope.isIPhone)) {
					...
					angular.element($window).bind('resize', twiddleScene);
				}
