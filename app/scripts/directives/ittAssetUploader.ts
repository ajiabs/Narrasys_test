...
interface AssetUploaderBindings extends ng.IComponentController{
  containerId?: string;
  episodeId?: string;
...
class AssetUploaderController implements AssetUploaderBindings {
  //bindings
  containerId: string;
  episodeId: string;
  callback: (data: any) => any;
  ...
  $onInit() {
    if (this.instructions === undefined) {
      this.manPage = `We support uploads of most common file formats, including .doc, .docx, .jpeg, .jpg, .pdf, .png,
      .ppt, .pptx, .rtf, .txt, and .zip.`;
    } else {
    ...
    if (this.mimeTypes == null) {
      ...
      this.mimeTypesArr = (this.mimeTypes as string).split(',');
    }
  ...
  $postLink() {
    this.$timeout(() => { // need to wait for the DOM
      ...
      this.droptarget[0].addEventListener('drop', (ev: any) => this.handleDrop(ev));
      this.droptarget[0].addEventListener('dragover', (ev: any) => this.handleDragOver(ev));
      this.droptarget[0].addEventListener('dragenter', (ev: any) => this.handleDragEnter(ev));
      this.droptarget[0].addEventListener('dragleave', (ev: any) => this.handleDragLeave(ev));
    });
  }
