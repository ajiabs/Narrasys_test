	...
	function ittNarrativeEditor() {
	    return {
			...
			template: [
				...
				'		</div>',
				'		<label id="nName">Name of Narrative</label><span ng-if="nEditForm.name.$invalid" class="narrative__invalid"> Required</span>',
				'		<input for="nName" type="text" name="name" placeholder="Add Narrative Name" ng-model="nEditor._narrative.name.en" required>',
				'		<label for="nDescription">Description</label><span ng-if="nEditForm.description.$invalid" class="narrative__invalid"> Required</span>',
				'		<textarea id="nDescription" name="description" placeholder="Add a Description" ng-model="nEditor._narrative.description.en" required></textarea>',
				'		<label for="nPath">Path</label><span ng-if="nEditForm.path.$invalid" class="narrative__invalid"> Required</span>',
				'		<input id="nPath" type="text" name="path" placeholder="link-to-narrative" ng-model="nEditor._narrative.path_slug.en" required/>',
				'		<label for="nSupportUrl">Support Url</label><span ng-if="nEditForm.supportUrl.$invalid" class="narrative__invalid"> Not a valid URL</span>',
