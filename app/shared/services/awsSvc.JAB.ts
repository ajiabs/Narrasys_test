...
export class awsSvc implements awsServices {
  ...
  startNextUpload(assetEndpoint) {
    ...
    if (this.files.length > this.fileIndex && !this.fileBeingUploaded) {
      ...
      this.ensureUniqueFilename().then(function () {
  ...
  putObject() {
    ...
    this.getUploadSession().then(function putObject() {
      ...
      this.getMD5ForFileOrBlob(this.fileBeingUploaded, 'base64').then(function (md5) {
        ...
        this.currentRequest.on('httpUploadProgress', function (progress) {
          this.deferredUpload.notify({
  ...
  uploadBigFile() {
    ...
    this.deferredUpload = this.$q.defer();
    this.createMultipartUpload().then(this.prepareUploadParts).then(function startUpload() {
      for (var i = 0; i < this.MAX_SIMUL_PARTS_UPLOADING; i++) {
        this.startNextUploadPart();
      }
    });
